...
var DraggableBackgroundWrapper = function (_React$Component) {
  ...
  DraggableBackgroundWrapper.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
    var wasOver = this.props.isOver;
    var isOver = nextProps.isOver;

    if (isOver && !wasOver) {
      var _context = this.context,
          onEventResize = _context.onEventResize,
          dragDropManager = _context.dragDropManager;
    ...
    drop: function drop(_, monitor, _ref) {
      ...
      if (monitor.getItemType() === 'resize') {
        ...
        switch (event.type) {
          case 'resizeT':
            {
              return onEventResize('drop', { event: event, start: value, end: event.end });
            }
          case 'resizeB':
            {
              return onEventResize('drop', { event: event, start: event.start, end: value });
            }
          case 'resizeL':
            {
              return onEventResize('drop', { event: event, start: value, end: event.end });
          ...
          case 'resizeR':
            {
              var nextEnd = _dates2.default.add(value, 1, 'day');
              return onEventResize('drop', { event: event, start: event.start, end: nextEnd });
            }
        }

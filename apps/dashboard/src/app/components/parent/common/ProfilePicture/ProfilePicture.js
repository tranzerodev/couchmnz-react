  ...
  */
  ...
  handleCropImage() {
    ...
    if (typeof this.props.onSelect === 'function') {
      ...
      canvasElement.toBlob(blob => {
        ...
        this.props.addChildPicture(formData, {parentId: this.props.profile.data.parentId, childId: this.props.profile.data.id});
      }, 'image/jpeg', 0.95);
    }
    this.setState({
  ...
  static get propTypes() {
    return {
      ...
      addChildPicture: PropTypes.func,
...
const mapStateToProps = state => {
  const {parent, profile} = state;
  const {picture} = parent;
  return {
    profile,
    picture
  };
...
const mapDispatchToProps = dispatch => {
  return {
    addChildPicture: (formData, params) => dispatch(addChildPicture(formData, params)),

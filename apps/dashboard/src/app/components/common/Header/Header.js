...
import {
  DASHBOARD_SSP, DASHBOARD_SCHEDULES, WELCOME, DASHBOARD_MANAGE_SPORT_SPECIALITY, 
  DASHBOARD_MANAGE_SPORT_TRAINING_PREFERENCES, DASHBOARD_MANAGE_SPORT_PRICING, 
  DASHBOARD_MANAGE_SPORT_TRAINING_LOCATIONS, DASHBOARD_MANAGE_SPORT_SESSIONS, 
  DASHBOARD_MANAGE_SPORT_BIOGRAPHY, DASHBOARD_MANAGE_SPORT_LISTING, 
  DASHBOARD_MANAGE_SPORT_MEDIA, DASHBOARD_ACCOUNT_BUSINESS_MODEL, 
  DASHBOARD_ACCOUNT_BOOKING_PREFERENCE, DASHBOARD_ACCOUNT_DETAILS, 
  DASHBOARD_ACCOUNT_PAYOUT_DETAILS, DASHBOARD_LINK, SHOPPING_CART, DASHBOARD,
...
import SportsSwitcherDropDown from '../../ssp/isp/common/SportsSwicherDropDown';
import HeaderSearch from '../HeaderSearch';
import * as RouterPaths from '../../../constants/RouterPaths';
import {notNull, notFetched} from '../../../validators/common/util';
import {fetchShoppingCartItemsCount, fetchMessagesMetadata} from '../../../actions';
...
class Header extends Component {
  constructor(props) {
    ...
    this.renderMobileNavBar = this.renderMobileNavBar.bind(this);
    this.renderMobileLogo = this.renderMobileLogo.bind(this);
    this.renderDashboardNav = this.renderDashboardNav.bind(this);
    this.renderSportNav = this.renderSportNav.bind(this);
    this.renderAccountNav = this.renderAccountNav.bind(this);
    this.getProfilePicture = this.getProfilePicture.bind(this);
    this.renderCartIcon = this.renderCartIcon.bind(this);
    this.handleShoppincart = this.handleShoppincart.bind(this);
    this.handleDashboardLink = this.handleDashboardLink.bind(this);
    this.renderMessagingIcon = this.renderMessagingIcon.bind(this);
    this.handleMessagingClick = this.handleMessagingClick.bind(this);
  }

  componentDidMount() {
    const {cartItemCountStatus} = this.props;
    if (notFetched(cartItemCountStatus)) {
      this.props.fetchShoppingCartItemsCount();
    }
  }
  ...
  renderMobileNavBar() {
    ...
    const isActive = ((profile.status === FULFILLED && profile.data.isActive === appConstants.profileActiveFlages.active) ||
      (profile.status === FULFILLED && profile.data && profile.data.profile && profile.data.profile.isActive === appConstants.profileActiveFlages.active));
    const logo = this.renderMobileLogo();
    return (
      <ul className="uk-nav uk-nav-dropdown" data-uk-nav="{multiple:true}">
        ...
        <li><a href={DASHBOARD_PATH()}>{this.props.p.t('DashboardNav.dashboard')}<svg className="cl-arrow-right" xmlns="http://www.w3.org/2000/svg" viewBox="-9499.75 -22752.947 9.5 18.293"><path data-name="Path 184" className="cl-arrow-path" d="M0-.2l8.4,8,8.4-8" transform="translate(-9498.801 -22735.405) rotate(-90)"/></svg></a></li>
  ...
  renderCartIcon() {
    ...
    return (
      <li>
        <a onClick={this.handleShoppincart} target="_self">
    ...
    return (
      <ul className="uk-nav uk-nav-dropdown" data-uk-nav="{multiple:true}">
        <li className="cl-mobile-main-menu-logo">
          <NavLink to={WELCOME} className="cl-logo-link">
  ...
  renderDashboardNav() {
    const sportName = this.props.currentSport.status === FULFILLED && this.props.currentSport.data.name;
    return (
      <ul className="uk-nav uk-nav-dropdown" data-uk-nav="{multiple:true}">
        {/* <li className="cl-select-role">
          <label htmlFor="cl-select-role">{this.props.p.t('Header.sign_out')}</label>
          <SportsSwitcherDropDown/>
        </li>*/}
                <li>
                ...
                <li>
                  <a href={ACCOUNTS_PATH('accountBalance')}>{this.props.p.t('messages')}</a>
                </li>                
        {/* <li>
          <NavLink to={DASHBOARD_SSP}>{this.props.p.t('DashboardNav.dashboard')}</NavLink>
        </li>
        <li>
        ...
        <li className="uk-parent">
          <a href="#">{this.props.p.t('DashboardNav.account')}</a>
          {
  ...
  renderSportNav() {
    return (
      <ul className="uk-nav-sub">
        <li><NavLink to={DASHBOARD_MANAGE_SPORT_SPECIALITY} >{this.props.p.t('DashboardProfileSideNav.sports_and_certificate')}</NavLink></li>
        <li><NavLink to={DASHBOARD_MANAGE_SPORT_TRAINING_PREFERENCES} >{this.props.p.t('DashboardProfileSideNav.training_preference')}</NavLink></li>
        <li><NavLink to={DASHBOARD_MANAGE_SPORT_PRICING} >{this.props.p.t('DashboardSessionSideNav.set_pricing')}</NavLink></li>
        <li><NavLink to={DASHBOARD_MANAGE_SPORT_TRAINING_LOCATIONS} >{this.props.p.t('DashboardSessionSideNav.training_locations')}</NavLink></li>
        <li><NavLink to={DASHBOARD_MANAGE_SPORT_SESSIONS} >{this.props.p.t('DashboardSessionSideNav.define_sessions')}</NavLink></li>
        <li><NavLink to={DASHBOARD_MANAGE_SPORT_BIOGRAPHY} >{this.props.p.t('DashboardProfileSideNav.biography')}</NavLink></li>
        <li><NavLink to={DASHBOARD_MANAGE_SPORT_LISTING} >{this.props.p.t('DashboardProfileSideNav.listing_details')}</NavLink></li>
        <li><NavLink to={DASHBOARD_MANAGE_SPORT_MEDIA} >{this.props.p.t('DashboardProfileSideNav.photos_and_videos')}</NavLink></li>
      </ul>
    );
  }
  ...
  renderAccountNav() {
    return (
      <ul className="uk-nav-sub">
        <li>
          <NavLink to={DASHBOARD_ACCOUNT_BUSINESS_MODEL}>{this.props.p.t('DashboardAccountSideNav.business_model')}</NavLink>
        </li>
        <li>
          <NavLink to={DASHBOARD_ACCOUNT_BOOKING_PREFERENCE}>{this.props.p.t('DashboardAccountSideNav.booking_preferences')}</NavLink>
        </li>
        <li>
          <NavLink to={DASHBOARD_ACCOUNT_DETAILS}>{this.props.p.t('DashboardAccountSideNav.account_details')}</NavLink>
        </li>
        <li >
  ...
  getProfilePicture(userProfiles) {
    if (userProfiles.selectedProfile) {
      switch (userProfiles.selectedProfile.type) {
        case appConstants.userProfileTypes.ATHLETE: {
          return notNull(this.props.athletePicture) && this.props.athletePicture.url ? this.props.athletePicture.url : PROFILE_SSP_DEFAULT;
        }
        case appConstants.userProfileTypes.ISP: {
          return this.props.displayPicture && this.props.displayPicture.picture ? this.props.displayPicture.picture + '?date=' + (new Date()) : PROFILE_SSP_DEFAULT;
        }
        case appConstants.userProfileTypes.PARENT: {
          return notNull(this.props.parentPicture) && this.props.parentPicture.url ? this.props.parentPicture.url : PROFILE_SSP_DEFAULT;
        }
        default: {
          return PROFILE_SSP_DEFAULT;
        }
      }
    }
    return PROFILE_SSP_DEFAULT;
  ...
  renderMessagingIcon() {
    ...
    const {unread} = metadata;
    return (
      <li>
        <a href={MESSAGES_PATH()} target="_self">
          <svg className="cl-icon-messages" xmlns="http://www.w3.org/2000/svg" viewBox="-3431 288.777 29.933 22.223">
            <g data-name="Group 90" transform="translate(-3430 289.777)">
              <path className="cl-icon-messages-1" d="M1,4.785V17.438a3.707,3.707,0,0,0,3.619,3.785h20.7a3.7,3.7,0,0,0,3.619-3.785V4.785A3.707,3.707,0,0,0,25.315,1H4.619A3.7,3.7,0,0,0,1,4.785Z" transform="translate(-1 -1)"/>
              <path className="cl-icon-messages-2" d="M2.372,1.8,4.22,3.425l9.107,8.01a2.845,2.845,0,0,0,3.684-.015l9.943-8.884" transform="translate(-0.717 -0.789)"/>
            </g>
          </svg>
          {unread > 0 &&
          <span className="cl-badge">{unread}</span>
          }
        </a>
      </li>
    );
  ...
  render() {
    const navBar = this.renderMobileNavBar();
    const {profile, userProfiles} = this.props;
    const isActive = ((profile.status === FULFILLED && profile.data.isActive === appConstants.profileActiveFlages.active) || (profile.status === FULFILLED && profile.data && profile.data.profile && profile.data.profile.isActive === appConstants.profileActiveFlages.active));
    const picture = this.getProfilePicture(userProfiles);
    return (
      <header className="cl-header-ab">
        <div className="uk-grid uk-grid-collapse">
          <div className="uk-width-6-10 uk-width-small-6-10 uk-width-medium-3-10 uk-width-large-2-10 uk-width-xlarge-2-10">
            <a href={config.homePage} className="cl-logo-link">
              <svg className="cl-logo" xmlns="http://www.w3.org/2000/svg" viewBox="424.098 1014 169.902 40.108">
                <g transform="translate(424 1013.717)">
                  <path className="cl-logo-001" d="M101.267,27.128a47.481,47.481,0,0,1,1.308-4.648c1.034-2.642,2.207-5.232,3.387-7.813a2.131,2.131,0,0,1,1.145-1.128,2.424,2.424,0,0,1,1.8.273c.251.174.139,1.046.007,1.556a4.658,4.658,0,0,1-.752,1.376,31.622,31.622,0,0,0-4.015,9.344,3.67,3.67,0,0,0-.132.962c-.01,1.106.455,1.493,1.429,1.019.832-.407,2.141-.507,1.787-2.08-.061-.269.528-.941.875-.988a2.11,2.11,0,0,1,1.6.445,23.613,23.613,0,0,1,1.462,2.229,16.623,16.623,0,0,0,1.509,1.561c.126-.077.254-.154.379-.231-.118-.814-.238-1.628-.353-2.442-.286-2.01-.62-4.016-.838-6.033a7.828,7.828,0,0,1,1.879-6.23,3.717,3.717,0,0,1,4.185-1.168,3.217,3.217,0,0,1,2.371,3.173c.013.155.041.308.054.4.759.2,1.459.378,2.152.581.755.222,1.163.023,1.455-.781.939-2.591,1.946-5.16,2.98-7.716.692-1.713,1.712-2,3.427-1.043-.426,3.68-3.125,6.463-4.168,10.393,1.8-.608,3.346-1.117,4.879-1.654.342-.118.675-.5.97-.461A12.217,12.217,0,0,1,134,16.6c-.3.47-.5,1.2-.925,1.366-2.106.83-4.248,1.589-6.424,2.213a2.322,2.322,0,0,0-1.817,1.72c-.745,2.613-1.5,5.222-2.235,7.837-.338,1.206-.9,1.661-1.845,1.443-1.047-.241-1.421-.9-1.048-2.174q1.14-3.913,2.424-7.783c.23-.691.218-1.079-.443-1.5a13.466,13.466,0,0,1-1.693-1.527c-.088.155-.224.369-.333.6-.389.8-.9,1.267-1.81.7a1.324,1.324,0,0,1-.55-1.949,4.147,4.147,0,0,0,.281-1.86c-.01-.266-.4-.629-.691-.731-.179-.063-.549.243-.752.458-2.011,2.129-1.713,4.666-1.279,7.24.335,2,.648,4.01.9,6.024a2.419,2.419,0,0,1-3.529,2.575,32.325,32.325,0,0,1-3.506-2.318,19.833,19.833,0,0,1-3.762,1.949c-1.816.52-2.463-1.119-3.244-2.393a2.123,2.123,0,0,1-.468.281c-2.881.809-5.76,1.629-8.651,2.4-.859.228-1.76.291-2.639.449a3.375,3.375,0,0,1-3.633-1.893,2.093,2.093,0,0,1-.022-1.614c1.826-4.72,3.674-9.434,5.6-14.116.815-1.987,1.846-3.885,2.724-5.847a1.183,1.183,0,0,1,1.845-.672c.835.384,1.961.562,1.267,1.913-.845,1.646-1.852,3.218-2.579,4.913-1.835,4.285-3.563,8.617-5.3,12.945-.164.409-.257,1.185-.054,1.329a1.879,1.879,0,0,0,1.448.149A63.377,63.377,0,0,0,97.373,26.7c1.5-.621,2.791-.763,3.894.432" transform="translate(35.902 2.611)"/>
                  <path className="cl-logo-002" d="M91.871,31.781c-.24,1.186-1.2,1.2-2,1.287a2.593,2.593,0,0,1-2.626-1.8,8.973,8.973,0,0,1-.137-3.477c.408-2.048,1.1-4.039,1.776-6.4-.834,1.032-1.5,1.8-2.11,2.622-1.666,2.254-3.267,4.558-4.968,6.786-1.021,1.338-2.095,1.441-3.34.391A1.517,1.517,0,0,1,78.123,30a24.914,24.914,0,0,1,.68-2.67c-1.36,1.048-2.4,1.961-3.543,2.7A15.076,15.076,0,0,1,71.526,32a3.637,3.637,0,0,1-4.6-4.408,11.243,11.243,0,0,1,.646-2.341,26.925,26.925,0,0,1,5.743-8.683c1.671-1.573,2.808-1.8,5.267-.4a1.623,1.623,0,0,1,.707,2.195c-.33.953-.653,1.911-1.056,2.834a1.061,1.061,0,0,1-.757.573,1.876,1.876,0,0,1-1.57-2.619,2.62,2.62,0,0,0,.225-1.453,2.792,2.792,0,0,0-.654.377,24.411,24.411,0,0,0-6.008,9.629,7.077,7.077,0,0,0-.084,1.864,5.682,5.682,0,0,0,1.97-.105,30.75,30.75,0,0,0,3.488-1.947c1.436-.869,2.62-2.214,4.481-2.278.172-.006.378-.353.492-.578,3.31-6.6,6.6-13.21,9.943-19.789a1.979,1.979,0,0,1,1.407-.991c.576,0,1.493.428,1.636.873a2.437,2.437,0,0,1-.521,1.935C87.5,12.436,85.244,19.5,82.2,26.173c1.176-1.505,2.317-3.041,3.538-4.51,1.428-1.72,2.919-3.389,4.4-5.064.264-.3.605-.721.929-.737a3.785,3.785,0,0,1,2.274.3c.318.239.133,1.411-.1,2.069-.939,2.669-2.024,5.286-2.965,7.953a9.132,9.132,0,0,0-.368,2.523c-.107,1.935.163,2.333,1.963,3.071" transform="translate(27.817 1.217)"/>
          ...
          <div className="uk-width-4-10 uk-width-small-4-10 uk-width-medium-7-10 uk-width-large-8-10 uk-width-xlarge-4-10">
            <ul className="navigation uk-hidden-small">
              ...
              {isActive &&
              <li className="active"><a target="_self" href={DASHBOARD_PATH()}>{this.props.p.t('Header.myActivity')}</a></li>}
              <li data-uk-dropdown="{mode:'click', pos:'bottom-right'}">
                <a target="_self"><span className="cl-icon-profile"><img src={picture}/></span></a>
                <div className="uk-dropdown cl-dropdown-profile">
                  <ul>
                    {/* <li><a target="_self">Profile<svg className="cl-arrow-right" xmlns="http://www.w3.org/2000/svg" viewBox="-9499.75 -22752.947 9.5 18.293"><path data-name="Path 184" className="cl-arrow-path" d="M0-.2l8.4,8,8.4-8" transform="translate(-9498.801 -22735.405) rotate(-90)"/></svg></a></li>
                    <li><a target="_self">Account<svg className="cl-arrow-right" xmlns="http://www.w3.org/2000/svg" viewBox="-9499.75 -22752.947 9.5 18.293"><path data-name="Path 184" className="cl-arrow-path" d="M0-.2l8.4,8,8.4-8" transform="translate(-9498.801 -22735.405) rotate(-90)"/></svg></a></li> */}
                    <li><a href="https://www.coachlist.com/dashboard">{this.props.p.t('DashboardNav.dashboard')}<svg className="cl-arrow-right" xmlns="http://www.w3.org/2000/svg" viewBox="-9499.75 -22752.947 9.5 18.293"><path data-name="Path 184" className="cl-arrow-path" d="M0-.2l8.4,8,8.4-8" transform="translate(-9498.801 -22735.405) rotate(-90)"/></svg></a></li>
                    <li><a href="https://www.coachlist.com/messages">{this.props.p.t('DashboardNav.messages')}<svg className="cl-arrow-right" xmlns="http://www.w3.org/2000/svg" viewBox="-9499.75 -22752.947 9.5 18.293"><path data-name="Path 184" className="cl-arrow-path" d="M0-.2l8.4,8,8.4-8" transform="translate(-9498.801 -22735.405) rotate(-90)"/></svg></a></li>
                    <li><a href="https://www.coachlist.com/bookings">{this.props.p.t('DashboardNav.bookings')}<svg className="cl-arrow-right" xmlns="http://www.w3.org/2000/svg" viewBox="-9499.75 -22752.947 9.5 18.293"><path data-name="Path 184" className="cl-arrow-path" d="M0-.2l8.4,8,8.4-8" transform="translate(-9498.801 -22735.405) rotate(-90)"/></svg></a></li>
                    <li><a href="https://www.coachlist.com/calendar">{this.props.p.t('DashboardNav.calendar')}<svg className="cl-arrow-right" xmlns="http://www.w3.org/2000/svg" viewBox="-9499.75 -22752.947 9.5 18.293"><path data-name="Path 184" className="cl-arrow-path" d="M0-.2l8.4,8,8.4-8" transform="translate(-9498.801 -22735.405) rotate(-90)"/></svg></a></li>
                    <li><a href="#" onClick={this.handleLogout}>{this.props.p.t('Header.sign_out')}<svg className="cl-arrow-right" xmlns="http://www.w3.org/2000/svg" viewBox="-9499.75 -22752.947 9.5 18.293"><path data-name="Path 184" className="cl-arrow-path" d="M0-.2l8.4,8,8.4-8" transform="translate(-9498.801 -22735.405) rotate(-90)"/></svg></a></li>
                  </ul>
                </div>
              </li>
            </ul>
            <div className="cl-mobile-main-menu">
              <button className="uk-button" data-uk-offcanvas="{target:'#cl-mobile-mainmenu-side', mode:'slide'}">
        ...
        <div id="cl-search-box" className="cl-Search-box uk-hidden"><HeaderSearch/></div>
      </header>
    );
  }
  static get propTypes() {
    return {
      ...
      athletePicture: PropTypes.object.isRequired,
      parentPicture: PropTypes.object.isRequired,
      displayPicture: PropTypes.object,
      currentSport: PropTypes.object.isRequired,
      cartItemCount: PropTypes.number.isRequired,
      cartItemCountStatus: PropTypes.string.isRequired,
      fetchShoppingCartItemsCount: PropTypes.func.isRequired,
...
Header.defaultProps = {
  history: {},
  profile: {},
  displayPicture: {picture: PROFILE_SSP_DEFAULT}
};

const mapStateToProps = state => {
  const {profile, displayPicture, currentSport, athlete, parent, shoppingCart, messages,} = state;
  ...
  return {
    ...
    parentPicture: parent.picture,
    cartItemCount: cartData.itemCount,
    cartItemCountStatus: cartData.itemCountStatus,
    metadata
  };
};
...
const mapDispatchToProps = dispatch => {
  return {
    fetchShoppingCartItemsCount: () => dispatch(fetchShoppingCartItemsCount()),
    fetchMessagesMetadata: profileId => dispatch(fetchMessagesMetadata(profileId))
  };
};

const HeaderPage = connect(mapStateToProps, mapDispatchToProps)(translate(Header));

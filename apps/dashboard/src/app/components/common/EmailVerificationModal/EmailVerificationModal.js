...
import {verifyAthleteEmailOTP, verifyAthleteEmail} from '../../../actions/index';
...
class BusinessEmailVerificationModal extends Component {
  ...
  componentWillReceiveProps(nextProp) {
    const {athleteEmailVerificationStatus, email2} = nextProp;
    if (athleteEmailVerificationStatus.status === FULFILLED && athleteEmailVerificationStatus.data.email === email2) {
      this.props.onClose();
    }
  }
  ...
  handleResend() {
    ...
    this.props.verifyAthleteEmail({profileId}, {email: email2});
  }
  ...
  handleSubmit() {
    ...
    if (this.state.validCode && this.state.validCode2) {
      this.props.verifyAthleteEmailOTP({profileId}, {
        email: email2,
  ...
  getErrorClass() {
    return (this.state.submitted && !this.state.validCode) || (this.props.athleteEmailVerificationStatus.status === REJECTED) ?
            ...
            <div className="uk-modal-body">
              <div className="uk-grid">
                ...
                <div className={this.getErrorClass()}>
                  <span className="error-text">{this.state.submitted && this.props.athleteEmailVerificationStatus.status === REJECTED && this.props.athleteEmailVerificationStatus.data.message === 'exists' ? t('BusinessEmailVerificationModal.validation_message.exists', {email: email2}) : ''}</span>
                </div>
              </div>
              <div className="uk-grid">
                <div className="uk-width-large-1-2 uk-width-medium-1-1 uk-width-small-1-1">
                  <div className={this.getErrorClass()}>
                    ...
                    {
                      this.props.athleteEmailVerificationStatus.data && this.props.athleteEmailVerificationStatus.data.message !== 'exists' && (
              ...
              <div className="uk-grid">
                <div className="uk-width-large-1-2 uk-width-medium-1-1 uk-width-small-1-1">
                  <div className={this.getErrorClass()}>
                    ...
                    {
                      this.props.athleteEmailVerificationStatus.data && this.props.athleteEmailVerificationStatus.data.message !== 'exists' && (
                        <span className="error-text">{this.state.submitted && !this.state.validCode2 && this.props.athleteEmailVerificationStatus.data && this.props.athleteEmailVerificationStatus.data.message !== 'exists' ?
                          t('BusinessEmailVerificationModal.validation_message.code_length') :
                          t('BusinessEmailVerificationModal.validation_message.match')}
                        </span>
                      )
  ...
  static get propTypes() {
    return {
      ...
      verifyAthleteEmailOTP: PropTypes.func.isRequired,
      profileId: PropTypes.string.isRequired,
      athleteEmailVerificationStatus: PropTypes.object.isRequired
    };
  }
}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    verifyAthleteEmailOTP: (params, data) => dispatch(verifyAthleteEmailOTP(params, data))
  };
};

const mapStateToProps = state => {
  const {userProfiles, athlete} = state;
  const {athleteEmailVerificationStatus} = athlete;
  return {

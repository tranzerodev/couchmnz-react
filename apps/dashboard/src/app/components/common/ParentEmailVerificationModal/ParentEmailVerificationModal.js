...
import {verifyParentEmailOTP, verifyParentEmail} from '../../../actions/index';
import {REJECTED, FULFILLED} from '../../../constants/ActionTypes';

class ParentEmailVerificationModal extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillReceiveProps(nextProp) {
    const {parentEmailVerificationStatus, email2} = nextProp;
    if (parentEmailVerificationStatus.status === FULFILLED && parentEmailVerificationStatus.data.email === email2) {
      this.props.onClose();
    }
  }
  ...
  handleResend() {
    ...
    this.props.verifyParentEmail({profileId}, {email: email2});
  }
  ...
  handleSubmit() {
    ...
    if (this.state.validCode && this.state.validCode2) {
      this.props.verifyParentEmailOTP({profileId}, {
        email: email2,
  ...
  getErrorClass() {
    return (this.state.submitted && !this.state.validCode) || (this.props.parentEmailVerificationStatus.status === REJECTED) ?
            ...
            <div className="uk-modal-body">
              <div className="uk-grid">
                ...
                <div className={this.getErrorClass()}>
                  <span className="error-text">{this.state.submitted && this.props.parentEmailVerificationStatus.status === REJECTED && this.props.parentEmailVerificationStatus.data.message === 'exists' ? t('BusinessEmailVerificationModal.validation_message.exists', {email: email2}) : ''}</span>
                </div>
              </div>
              <div className="uk-grid">
                <div className="uk-width-large-1-2 uk-width-medium-1-1 uk-width-small-1-1">
                  <div className={this.getErrorClass()}>
                    ...
                    {
                      this.props.parentEmailVerificationStatus.data && this.props.parentEmailVerificationStatus.data.message !== 'exists' && (
              ...
              <div className="uk-grid">
                <div className="uk-width-large-1-2 uk-width-medium-1-1 uk-width-small-1-1">
                  <div className={this.getErrorClass()}>
                    ...
                    {
                      this.props.parentEmailVerificationStatus.data && this.props.parentEmailVerificationStatus.data.message !== 'exists' && (
                        <span className="error-text">{this.state.submitted && !this.state.validCode2 && this.props.parentEmailVerificationStatus.data && this.props.parentEmailVerificationStatus.data.message !== 'exists' ?
                          t('BusinessEmailVerificationModal.validation_message.code_length') :
                          t('BusinessEmailVerificationModal.validation_message.match')}
                        </span>
                      )
  ...
  static get propTypes() {
    return {
      ...
      verifyParentEmailOTP: PropTypes.func.isRequired,
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    verifyParentEmailOTP: (params, data) => dispatch(verifyParentEmailOTP(params, data))
  };
};

const mapStateToProps = state => {
  const {userProfiles, parent} = state;
  const {parentEmailVerificationStatus} = parent;
  return {
    profileId: userProfiles.selectedProfile.id,
    parentEmailVerificationStatus};
};

export default connect(mapStateToProps, mapDispatchToProps)(withRouter(translate(ParentEmailVerificationModal)));

...
class ISPTeamPricingClass extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const basePrice = price.prices && price.prices.length && price.prices[0].price >= 0 ? price.prices[0].price : 0.0;
        ...
        <div className="accordionOuter trainingAccordian" style={{display: this.props.training && this.props.training.length ? 'block' : 'none'}}>
          ...
          <div className="uk-accordion pricingAccordion" data-uk-accordion="{showfirst: true}">
            <div className={'uk-accordion-title ' + price} onClick={this.handleToggle}>
              <h3> {this.props.p.t('ISPTeamPricing.priceForTeams')}<br/>
                {
                  price.prices && price.prices.length ? price.prices.map((p, i) => {
                    return (
                      <span key={i} className="lines">
                        <strong>${p.price ? p.price : 0.0}</strong> {this.props.p.t('ISPTeamPricing.perAthletePerSession', {session: singularSession})} ({this.props.p.t('ISPTeamPricing.teamSize')}: {p.min} to {p.max})
                      </span>);
                  }) : <span/>
                }
              </h3>
            ...
            <div className="uk-accordion-content" style={{display: pricing === 'uk-active' ? 'block' : 'none'}}>
              <div className="accordianWrap">
                <p>{this.props.p.t('ISPTeamPricing.perAthletePerSessionMessage', {session: singularSession})}</p>
                      ...
                      <div key={i} className={'uk-grid ' + (i ? 'mt20 marginSec' : '')}>
                        <div className="uk-width-xlarge-2-10 uk-width-large-2-10 uk-width-medium-1-4  uk-width-small-1-1">
                          <div className={isValidPrice === false && validation.valid === false && submitted ? 'field-holder error' : 'field-holder'}>
                            <label>{this.props.p.t('ISPTeamPricing.pricePerAthlete')}</label>
                            <div className="dollardiv">
                              ...
                              <span className="error-text">{this.props.p.t('ISPTeamPricing.validation_messages.pricing')}</span>
                            </div>
                          </div>
                        </div>
                        <div className="uk-width-xlarge-2-10 uk-width-large-2-10 uk-width-medium-1-4  uk-width-small-1-1">
                          <div className={(isValidMin === false) && validation.valid === false && submitted ? 'field-holder error' : 'field-holder'}>
                            <label>{this.props.p.t('ISPTeamPricing.minTeamSize')}</label>
                            ...
                            <span className="error-text">{this.props.p.t('ISPTeamPricing.validation_messages.min')}</span>
                          </div>
                        </div>
                        <div className="uk-width-xlarge-2-10 uk-width-large-2-10 uk-width-medium-1-4  uk-width-small-1-1">
                          <div className={(isValidMax === false || isValidMinMax === false) && validation.valid === false && submitted ? 'field-holder error' : 'field-holder'}>
                            <label>{this.props.p.t('ISPTeamPricing.maxTeamSize')}</label>
                            ...
                            {
                              (isValidMax === false) ? <span className="error-text">{this.props.p.t('ISPTeamPricing.validation_messages.max')}</span> : <span className="error-text">{this.props.p.t('ISPTeamPricing.validation_messages.minMaxSize')}</span>
                            }
                          </div>
                        </div>
                        <div className="uk-width-xlarge-2-10 uk-width-large-2-10 uk-width-medium-1-4  uk-width-small-1-1">
                ...
                <div className="uk-grid mt20">
                  <div className="uk-width-xlarge-1-1 uk-width-large-1-1 uk-width-medium-1-1  uk-width-small-1-1">
                    <div className="addCertification">
                      <a onClick={this.handleAddGroupSize}>{this.props.p.t('ISPTeamPricing.addAnother')}</a>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className={'uk-accordion-title ' + discount} onClick={this.handleDiscount}>
              <h3> {this.props.p.t('ISPTeamPricing.defineVolumeDiscounts')} <span>({this.props.p.t('ISPTeamPricing.optional')})</span><br/>
                <span className="lines"><strong>{volumeDiscount && volumeDiscount.length ? volumeDiscount.length : 0} {name} {this.props.p.t('ISPTeamPricing.packages')}</strong></span><br/>
                {
                  volumeDiscount.map((discount, i) => {
                    const currentPrice = basePrice;
                    return (
                      <span key={i} className="lines">
                        {discount.numberOfSessions} {isNumber(discount.numberOfSessions) && parseInt(discount.numberOfSessions, 10) === 1 ? singularSession : pluralSession} {this.props.p.t('ISPTeamPricing.for')} ${(currentPrice - (currentPrice * discount.discount / 100.0)).toFixed(2)} {this.props.p.t('ISPTeamPricing.perSession', {session: singularSession})} ({parseFloat(discount.discount).toFixed(0)}% {this.props.p.t('ISPTeamPricing.discount')}; {this.props.p.t('ISPTeamPricing.athletesSave')} ${(currentPrice * (parseFloat(discount.discount) ? (parseFloat(discount.discount) / 100.0) : 0.0) * parseInt(discount.numberOfSessions, 10)).toFixed(2)})
                      </span>
                    );
                  })
                }
            ...
            <div className="uk-accordion-content" style={{display: discount === 'uk-active' ? 'block' : 'none'}}>
              <div className="accordianWrap">
                <h4>{this.props.p.t('ISPTeamPricing.recommendedDiscountPoints')}</h4>
                        ...
                        <div key={i} className={this.props.packageItemClassName}>
                          <div className="packageItem" value={discount.id} name={i} onClick={this.props.onActivateDiscount}>
                            ...
                            <div className="emptyBorder">
                              <p><a>{discount.discount}%</a> {this.props.p.t('ISPTeamPricing.discount')}<br/> {this.props.p.t('ISPTeamPricing.athletesSave')} ${((currentPrice * discount.discount / 100) * discount.numberOfSessions).toFixed(2)}</p>
                            </div>
                            <p className="sessionPrice"><a >${(currentPrice - (currentPrice * discount.discount / 100)).toFixed(2)}</a> {this.props.p.t('ISPTeamPricing.perSession', {session: singularSession})}</p>
                          </div>
                ...
                <h4 className="pdnone"><a onClick={this.handleCreatePackage}>{this.props.p.t('ISPTeamPricing.createAnotherPackage')} <span>{this.props.p.t('ISPTeamPricing.createPackagesMessage')}</span></a></h4>
                <p className="pdnone"><a/></p>
              </div>
            </div>

            <div className={'uk-accordion-title ' + skillLevel} onClick={this.handleSkilllevel}>
              <h3> {this.props.p.t('ISPTeamPricing.specificPricingMessage')} <span>({this.props.p.t('ISPTeamPricing.optional')})</span><br/> <span className="lines">{this.props.p.t('ISPTeamPricing.forExample')}: <strong>{this.props.p.t('ISPTeamPricing.specificPricingExample')}</strong></span></h3>
            </div>
...
ISPTeamPricingClass.defaultProps = {
  training: [],
...
const ISPTeamPricing = connect(mapStateToProps, mapDispatchToProps)(ISPTeamPricingClass);
export default translate(ISPTeamPricing);

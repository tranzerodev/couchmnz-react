...
class ISPIndividualPricingClass extends Component {
  constructor(props) {
    super(props);
        ...
        <div className="accordionOuter" style={{display: this.props.training && this.props.training.length ? 'block' : 'none'}}>
          ...
          <div className="uk-accordion pricingAccordion">
            ...
            <div className={'uk-accordion-title ' + pricing} onClick={this.handleToggle}>
              <h3>{this.props.p.t('ISPIndividualPricing.priceFor')} {name}<br/> <span className="lines"><strong>${basePrice}</strong> {this.props.p.t('ISPIndividualPricing.singleSession', {session: singularSession})}</span></h3>
            </div>
            <div className="uk-accordion-content" style={{display: pricing === 'uk-active' ? 'block' : 'none'}}>
              <div className="accordianWrap">
                <p>{this.props.p.t('ISPIndividualPricing.message', {session: singularSession})}</p>
                <div className={submitted && validation.prices === false ? 'field-holder error' : 'field-holder'}>
                  <div className="dollardiv">
                    <span className="dollar">$</span>
                    <input type="number" min={0} name={0} placeholder={this.props.p.t('ISPIndividualPricing.singleSessionPrice', {session: singularSession})} className="uk-form-width-small field-required" onChange={this.props.onPriceChange} value={basePrice}/>
                    <span className="error-text">{this.props.p.t('ISPIndividualPricing.validation_messages.pricing', {session: singularSession})}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className={'uk-accordion-title ' + discount} onClick={this.handleDiscount}>
              <h3> {this.props.p.t('ISPIndividualPricing.defineVolumeDiscounts')} <span>({this.props.p.t('ISPIndividualPricing.optional')})</span><br/>
                <span className="lines"><strong>{volumeDiscount && volumeDiscount.length ? volumeDiscount.length : 0} {name} {this.props.p.t('ISPIndividualPricing.packages')}</strong></span><br/>
                {
                  volumeDiscount.map((discount, i) => {
                    const currentPrice = basePrice;
                    return (
                      <span key={i} className="lines">
                        {discount.numberOfSessions} {isNumber(discount.numberOfSessions) && parseInt(discount.numberOfSessions, 10) === 1 ? singularSession : pluralSession} {this.props.p.t('ISPIndividualPricing.for')} ${(currentPrice - (currentPrice * discount.discount / 100.0)).toFixed(2)} {this.props.p.t('ISPIndividualPricing.perSession', {session: singularSession})} ({parseFloat(discount.discount).toFixed(0)}% {this.props.p.t('ISPIndividualPricing.discount')}; {this.props.p.t('ISPIndividualPricing.athletesSave')} ${(currentPrice * (parseFloat(discount.discount) ? (parseFloat(discount.discount) / 100.0) : 0.0) * parseInt(discount.numberOfSessions, 10)).toFixed(2)})
                      </span>
                    );
                  })
                }
            ...
            <div className="uk-accordion-content" style={{display: discount === 'uk-active' ? 'block' : 'none'}}>
              <div className="accordianWrap">
                <h4>{this.props.p.t('ISPIndividualPricing.recommendedDiscountPoints')}</h4>
                        ...
                        <div key={i} className={this.props.packageItemClassName}>
                          <div className="packageItem" value={discount.id} name={i} onClick={this.props.onActivateDiscount}>
                            ...
                            <div className="emptyBorder">
                              <p><a>{discount.discount}%</a> {this.props.p.t('ISPIndividualPricing.discount')}<br/> {this.props.p.t('ISPIndividualPricing.athletesSave')} ${((currentPrice * discount.discount / 100) * discount.numberOfSessions).toFixed(2)}</p>
                            </div>
                            <p className="sessionPrice"><a >${(currentPrice - (currentPrice * discount.discount / 100)).toFixed(2)}</a> {this.props.p.t('ISPIndividualPricing.perSession', {session: singularSession})}</p>
                          </div>
                ...
                <h4 className="pdnone"><a onClick={this.handleCreatePackage}>{this.props.p.t('ISPIndividualPricing.createAnotherPackage')} <span>{this.props.p.t('ISPIndividualPricing.createPackagesMessage')}</span></a></h4>
                <p className="pdnone"><a/></p>
              </div>
            </div>

            <div className={'uk-accordion-title ' + skillLevel} onClick={this.handleSkilllevel}>
              <h3> {this.props.p.t('ISPIndividualPricing.specificPricingMessage')} <span>({this.props.p.t('ISPIndividualPricing.optional')})</span><br/> <span className="lines">{this.props.p.t('ISPIndividualPricing.forExample')}: <strong>{this.props.p.t('ISPIndividualPricing.specificPricingExample')}</strong></span></h3>
            </div>
                        ...
                        <div className="uk-grid ">
                          ...
                          <div className="uk-width-xlarge-1-4 uk-width-large-1-4 uk-width-medium-3-10  uk-width-small-1-1">
                            <div className={validation.skillLevels && validation.skillLevels.length && validation.skillLevels[currentSkillIndex] === false && submitted ? 'field-holder error' : 'field-holder'}>
                              <input
                                type="number"
                                title={currentSkill.id}
                                name={skillLevel.id}
                                placeholder={this.props.p.t('ISPIndividualPricing.enterAmount')}
                                value={currentSkill.operand ? currentSkill.operand : ''}
                                onChange={this.props.onSkillLevelOperand}
                              />
                        ...
                        <div className="uk-grid ">
                          ...
                          <div className="uk-width-xlarge-1-4 uk-width-large-1-4 uk-width-medium-3-10  uk-width-small-1-1">
                            <div className={validation.ages && validation.ages.length && validation.ages[currentAgeIndex] === false && submitted ? 'field-holder error' : 'field-holder'}>
                              <input title={currentAge.id} type="number" name={age.id} placeholder={this.props.p.t('ISPIndividualPricing.enterAmount')} onChange={this.props.onAgesOperand} value={currentAge.operand ? currentAge.operand : ''}/>
...
ISPIndividualPricingClass.defaultProps = {
  training: [],
...
const ISPIndividualPricing = connect(mapStateToProps, mapDispatchToProps)(ISPIndividualPricingClass);

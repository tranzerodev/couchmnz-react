...
class ISPGroupPricingClass extends Component {
  constructor(props) {
    super(props);
        ...
        <div className="accordionOuter trainingAccordian" style={{display: this.props.training && this.props.training.length ? 'block' : 'none'}}>
          ...
          <div className="uk-accordion pricingAccordion" data-uk-accordion="{showfirst: true}">
            <div className={'uk-accordion-title ' + price} onClick={this.handleToggle}>
              <h3> {this.props.p.t('ISPGroupPricing.priceFor')} {name}<br/>
                {
                  price.prices && price.prices.length ? price.prices.map((p, i) => {
                    return (
                      <span key={i} className="lines">
                        <strong>${p.price ? p.price : 0.0}</strong> {this.props.p.t('ISPGroupPricing.perAthletePerSession', {session: singularSession})} ({this.props.p.t('ISPGroupPricing.perAthletePerSession', {session: singularSession})}: {p.min} to {p.max})
                      </span>);
                  }) : <span/>
                }
              </h3>
            ...
            <div className="uk-accordion-content" style={{display: pricing === 'uk-active' ? 'block' : 'none'}}>
              <div className="accordianWrap">
                <p>{this.props.p.t('ISPGroupPricing.perAthletePerSessionMessage', {session: singularSession})}</p>
                      ...
                      <div key={i} className={'uk-grid ' + (i ? 'mt20 marginSec' : '')}>
                        <div className="uk-width-xlarge-2-10 uk-width-large-2-10 uk-width-medium-1-4  uk-width-small-1-1">
                          <div className={isValidPrice === false && validation.valid === false && submitted ? 'field-holder error' : 'field-holder'}>
                            <label>{this.props.p.t('ISPGroupPricing.priceForGroups')}</label>
                            <div className="dollardiv">
                              ...
                              <span className="error-text">{this.props.p.t('ISPGroupPricing.validation_messages.pricing')}</span>
                            </div>
                          </div>
                        </div>
                        <div className="uk-width-xlarge-2-10 uk-width-large-2-10 uk-width-medium-1-4  uk-width-small-1-1">
                          <div className={(isValidMin === false) && validation.valid === false && submitted ? 'field-holder error' : 'field-holder'}>
                            <label>{this.props.p.t('ISPGroupPricing.minGroupSize')}</label>
                            ...
                            <span className="error-text">{this.props.p.t('ISPGroupPricing.validation_messages.min')}</span>
                          </div>
                        </div>
                        <div className="uk-width-xlarge-2-10 uk-width-large-2-10 uk-width-medium-1-4  uk-width-small-1-1">
                          <div className={(isValidMax === false || isValidMinMax === false) && validation.valid === false && submitted ? 'field-holder error' : 'field-holder'}>
                            <label>{this.props.p.t('ISPGroupPricing.maxGroupSize')}</label>
                            ...
                            {
                              (isValidMax === false) ? <span className="error-text">{this.props.p.t('ISPGroupPricing.validation_messages.max')}</span> : <span className="error-text">{this.props.p.t('ISPGroupPricing.validation_messages.minMaxSize')}</span>
                            }
                          </div>
                        </div>
                        <div className="uk-width-xlarge-2-10 uk-width-large-2-10 uk-width-medium-1-4  uk-width-small-1-1">
                ...
                <div className="uk-grid mt20">
                  <div className="uk-width-xlarge-1-1 uk-width-large-1-1 uk-width-medium-1-1  uk-width-small-1-1">
                    <div className="addCertification">
                      <a onClick={this.handleAddGroupSize}>{this.props.p.t('ISPGroupPricing.addAnother')}</a>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className={'uk-accordion-title ' + discount} onClick={this.handleDiscount}>
              <h3> {this.props.p.t('ISPGroupPricing.defineVolumeDiscounts')} <span>({this.props.p.t('ISPGroupPricing.optional')})</span><br/>
                <span className="lines"><strong>{volumeDiscount && volumeDiscount.length ? volumeDiscount.length : 0} {name} {this.props.p.t('ISPGroupPricing.packages')}</strong></span><br/>
                {
            ...
            <div className="uk-accordion-content" style={{display: discount === 'uk-active' ? 'block' : 'none'}}>
              <div className="accordianWrap">
                <h4>{this.props.p.t('ISPGroupPricing.recommendedDiscountPoints')}</h4>
                        ...
                        <div key={i} className={this.props.packageItemClassName}>
                          <div className="packageItem" value={discount.id} name={i} onClick={this.props.onActivateDiscount}>
                            ...
                            <div className="emptyBorder">
                              <p><a>{discount.discount}%</a> {this.props.p.t('ISPGroupPricing.discount')}<br/> {this.props.p.t('ISPGroupPricing.athletesSave')} ${((currentPrice * discount.discount / 100) * discount.numberOfSessions).toFixed(2)}</p>
                            </div>
                            <p className="sessionPrice"><a >${(currentPrice - (currentPrice * discount.discount / 100)).toFixed(2)}</a> {this.props.p.t('ISPGroupPricing.perSession', {session: singularSession})}</p>
                          </div>
                ...
                <h4 className="pdnone"><a onClick={this.handleCreatePackage}>{this.props.p.t('ISPGroupPricing.createAnotherPackage')} <span>{this.props.p.t('ISPGroupPricing.createPackagesMessage')}</span></a></h4>
                <p className="pdnone"><a/></p>
              </div>
            </div>

            <div className={'uk-accordion-title ' + skillLevel} onClick={this.handleSkilllevel}>
              <h3> {this.props.p.t('ISPGroupPricing.specificPricingMessage')} <span>({this.props.p.t('ISPGroupPricing.optional')})</span><br/> <span className="lines">{this.props.p.t('ISPGroupPricing.forExample')}: <strong>{this.props.p.t('ISPGroupPricing.specificPricingExample')}</strong></span></h3>
            </div>
...
ISPGroupPricingClass.defaultProps = {
  training: [],
...
const ISPGroupPricing = connect(mapStateToProps, mapDispatchToProps)(ISPGroupPricingClass);
export default translate(ISPGroupPricing);

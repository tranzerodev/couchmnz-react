...
class AddNewDegree extends Component {
  constructor(props) {
    super(props);
    this.state = {
      degreeName: '',
  ...
  handleSubmit() {
    ...
    if (this.state.valid && this.state.validYear) {
      const data = {
        ...
        type: appConstants.masterDataTypes.ispDegree,
        countryID: null,
        stateID: null,
        stateName: null,
        cityID: null,
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.addNewMasterData.status === FULFILLED && this.state.posted) {
      this.setState({posted: false, degreeName: '', year: '', validYear: false, valid: false, submitted: false, showSuccess: true});
    }
  }
  handleClose() {
    this.props.handleClose();
    this.setState({posted: false, degreeName: '', year: '', validYear: false, valid: false, submitted: false, showSuccess: false});
  }
  getYears() {
    const years = [];
  ...
  render() {
    ...
    const type = p.t('Biography.degree');
    return (
        ...
        <div id="adddergreeModal" className="degreeModal">
          <div className="uk-modal-dialog uk-modal-dialog-small">
            <div className="uk-modal-header">
              <h2>{this.props.p.t('AddNewDegree.addDegree')}</h2>
              {!this.state.showSuccess &&
              <p><strong>{this.props.p.t('AddNewDegree.note')}:</strong> {this.props.p.t('AddNewDegree.noteDesc')}</p>
              }
              ...
              <div className="uk-grid">
                ...
                <div className="uk-width-xlarge-1-1 uk-width-large-1-1 uk-width-medium-1-1  uk-width-small-1-1">
                  <div className={(this.state.submitted && this.state.valid === false) || (this.props.addNewMasterData.status === REJECTED) ? 'field-holder error' : 'field-holder'}>
                    <label>{this.props.p.t('AddNewDegree.name')}</label>
                    <input onChange={this.handleChange} type="text" name="" className="field-required" placeholder={this.props.p.t('AddNewDegree.placeholder')}/>
                    <span className="error-text">{this.props.addNewMasterData.status === REJECTED ? this.props.p.t('AddNewDegree.server_error') : this.props.p.t('AddNewDegree.validation_messages.degree_name')}</span>
                  </div>
                </div>
                }
              </div>
            ...
            <div className="uk-modal-footer">
              <div className="tableDiv">
                {!this.state.showSuccess &&
                <div className="lCol">
                  <a onClick={this.handleSubmit} className="general_btn">{this.props.p.t('AddNewDegree.add')}</a>
                </div>
                }
                {!this.state.showSuccess &&
                <div className="rCol">
                  <a onClick={this.props.handleClose} className="cancel">{this.props.p.t('AddNewDegree.cancel')}</a>
                </div>
                }
                {this.state.showSuccess &&
                <div className="rCol">
...
const AddNewDegreeModal = connect(mapStateToProps, mapDispatchToProps)(translate(AddNewDegree));

...
const ssrUrl = (argv.ssrUrl === undefined) ? 'https://dev2.coachlist.com' : argv.ssrUrl;
const sspSearchBaseUrl = (argv.sspSearchBaseUrl === undefined) ? 'https://dev2.coachlist.com/search' : argv.sspSearchBaseUrl;
const googleApiKey = (argv.googleApiKey === undefined) ? 'AIzaSyCxfm_wmEREVuIa-lZvIZ2gCfcwwrG7yyY' : argv.googleApiKey;
const stripePublicKey = (argv.stripePublicKey === undefined) ? 'pk_test_qoMHuBUGgp4dICMmNaqV6B9r' : argv.stripePublicKey;
...
gulp.task('replace-base-url', () => {
  ...
  console.log('sspSearchBaseUrl', sspSearchBaseUrl);
  ...
  gulp.src('src/app/config/config.json')
    ...
    .pipe(jsonModify({
      key: 'sspSearchBaseUrl',
      value: sspSearchBaseUrl
    }))
    .pipe(jsonModify({
      key: 'googleApiKey',
      value: googleApiKey
...
gulp.task('html-dev', () => {
  return gulp.src('public/**/*.html')
    .pipe(replace('%ASSETS_BASE_PATH%', assetsBasePath))
    .pipe(replace('%SEARCH_BASE_PATH%', sspSearchBaseUrl))
...
gulp.task('webpack-dev-server', callback => {
  ...
  new WebpackDevServer(webpack(myConfig), {
    ...
    gutil.log('[webpack-dev-server]', 'https://coachlist-master-kchan1288.c9users.io/');
  });
});
